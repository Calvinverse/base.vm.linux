  ...
  context 'create the template files for consul-template vault connection' do
    ...
    consul_template_vault_content = <<~CONF
      ...
      template {
        ...
        destination = "/etc/consul-template.d/conf/vault.hcl"
    ...
    consul_template_vault_token_template_content = <<~CONF
      {{ $hostname := (env "HOSTNAME") }}
      {{ if keyExists (printf "auth/services/templates/%s/secrets" $hostname) }}
      {{ key (printf "auth/services/templates/%s/secrets" $hostname) }}
      {{ end }}
    CONF
    it 'creates consul-template vault token template in the consul-template template directory' do
      expect(chef_run).to create_file('/etc/consul-template.d/templates/consul_template_vault_token.ctmpl')
        .with_content(consul_template_vault_token_template_content)
    end

    consul_template_vault_token_content = <<~CONF
      # This block defines the configuration for a template. Unlike other blocks,
      # this block may be specified multiple times to configure multiple templates.
      # It is also possible to configure templates via the CLI directly.
      template {
        ...
        # called the "Consul Template template". This option is required if not using
        # the `contents` option.
        source = "/etc/consul-template.d/templates/consul_template_vault_token.ctmpl"
    ...
    CONF
    it 'creates consul_template_vault_token.hcl in the consul-template template directory' do
      expect(chef_run).to create_file('/etc/consul-template.d/conf/consul_template_vault_token.hcl')
        .with_content(consul_template_vault_token_content)
    end
  end
end
